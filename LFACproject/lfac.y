%{
    ...
    void declarare_functie(char*tip,char*nume,char *arg)
    {
        ...
        else
        {
            ...
            printf("Functia are %d parametri. Tipurile parametrilor sunt: %s ",Functii[nr_functii].nr,Functii[nr_functii].tipuri);
            nr_functii++;
        }
    }
...
%}
    ...
    instructiune: expresie PV
                ...
                | TYPEOF PDESCHISA apeluri_functii PINCHISA PV
                {
                    char *x=tip_functie($3); printf("tipul expresiei este %s \n",x);
                }
                | EVAL PDESCHISA apeluri_functii PINCHISA PV
                {
                    char *x=tip_functie($3); printf("tipul expresiei este %s \n",x);
                }
                | apeluri_functii
    ...
    param_lista: ID { $$=malloc(300); $$[0]=0;
            ...
            | param_lista VIRGULA apeluri_functii {char *x;
                  x=tip_functie($3); 
                 strcat($$,x); strcat($$," ");}
